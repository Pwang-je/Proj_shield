...
public class CaseManager {
	...
	public ArrayList<CaseDto> loadCase(String nowPage, String name, String title) { // 인덱스에서 사건을 로드하는 매서드
		...
		try {
			...
			if (name == null && title == null) { // 처음 접속 혹은 페이지만 바뀔 때
				sql = "select * from cases order by no desc";
				conn = ds.getConnection();
				pstate = conn.prepareStatement(sql);
				result = pstate.executeQuery();
			} else if (title == null) { // 작성자를 검색했을 때
	...
	public ArrayList<String> loadVillian(String char1, String char2, String voca) {
		...
		try {
			...
			if(char2.equals("0~180")) {
				char1 = "heroes_power."+char1;
				String sql = "select villain.name from villain left outer join heroes_power on villain.name = heroes_power.name where ("+char1+" = '1') and (villain.height > ?) and (villain.height <= ?) and (villain.name like ?)";
				pstate = conn.prepareStatement(sql);
				...
				char1 = "heroes_power."+char1;
				String sql = "select villain.name from villain left outer join heroes_power on villain.name = heroes_power.name where ("+char1+" = '1') and (villain.height > ?) and (villain.name like ?)";
				pstate = conn.prepareStatement(sql);
